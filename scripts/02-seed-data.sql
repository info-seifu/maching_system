-- Insert sample admin users (these would need to be created in Supabase Auth first)
...
-- Get the actual user ID for the existing user
DO $$
DECLARE
...
BEGIN
    ...
    IF existing_user_id IS NOT NULL THEN
        -- Insert a sample student profile for the existing user
        INSERT INTO public.students (user_id, name, furigana, birthday, gender, expertise) VALUES
...
INSERT INTO public.students (id, name, furigana, birthday, gender, expertise) VALUES
...
INSERT INTO public.match_rules (id, name, description) VALUES
('990e8400-e29b-41d4-a716-446655440001', 'デフォルトマッチングルール', '標準的なマッチングアルゴリズム')
ON CONFLICT (id) DO UPDATE SET
    name = EXCLUDED.name,
    description = EXCLUDED.description;

INSERT INTO public.match_rule_items (rule_id, attribute_name, weight) VALUES
